...
from fbcrawl.items import FbcrawlItem, parse_date, parse_date2,url_strip
...
class FacebookSpider(scrapy.Spider):
    ...
    name = 'fb'
    custom_settings = {
        'FEED_EXPORT_FIELDS': ['post_id','source','date','post_text','content', \
                               'reactions','comments','likes','haha','love','wow', \
                               'sigh','angry','care','url'],
    ...
    def parse_page(self, response):
        ...
        for post in response.xpath("//article[contains(@data-ft,'top_level_post_id')]"):     
            ...
            story_url = url_strip(post.xpath(".//a[contains(@href,'footer')]/@href").extract())
            ...
            self.count -= 1
            yield scrapy.Request(temp_post, self.parse_post, priority = self.count, meta={'item':new,'current_date':current_date})   
    ...
    def parse_post(self,response):
        ...
        new.add_xpath('post_text','//div[@data-ft]//p//text() | //div[@data-ft]/div[@class]/div[@class]/text()')
        ...
        if not check_reactions:
            ...
            yield scrapy.Request(reactions, callback=self.parse_reactions, meta={'item':new,'current_date':response.meta['current_date']})
